...
// মিউজিক বন্ধ হলে localStorage থেকে সেটিং মুছে ফেলা
...
function startTimer() {
    ...
    if(timeLeft == 28){
        quizPage.style.backgroundColor = "red";
    }
}
...
answerButtons.forEach(button => {
    button.addEventListener("click", () => {
        ...
        userSelections[currentQuestion] = selectedAnswer; // Store selection in localStorage
        ...
        if (selectedAnswer === quizData[currentQuestion].correct) {
            ...
            wrongSound.play();
            highlightCorrectAnswer(); // Show correct answer when wrong
        }
        ...
        scoreDisplay.innerText = `Score: ${score}`;
        localStorage.setItem("userSelections", JSON.stringify(userSelections)); // Store selections
    });
});

// Highlight correct answer when user selects wrong one
